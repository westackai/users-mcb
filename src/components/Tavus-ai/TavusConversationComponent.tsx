...
const TavusConversationComponent: React.FC<TavusConversationProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-gradient-to-br from-gray-900 via-blue-900 to-indigo-900 z-50">
      ...
      {remoteParticipantIds?.length > 0 ? (
        <div className="absolute inset-0 w-full h-full flex items-center justify-center bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-900">
          {/* AI Avatar Container with Professional Styling */}
          <div className="relative w-full max-w-4xl h-full max-h-[80vh] mx-auto flex items-center justify-center">
            {/* Main Avatar Video */}
            <div className="relative w-full h-full rounded-2xl overflow-hidden shadow-2xl border border-white/10 bg-black/20 backdrop-blur-sm">
              <video
                ref={(videoElement) => {
                  if (videoElement && daily) {
                    const track = daily.participants()[remoteParticipantIds[0]]?.tracks?.video;
                    if (track?.persistentTrack) {
                      videoElement.srcObject = new MediaStream([track.persistentTrack]);
                    }
                  }
                }}
                autoPlay
              ...
              <div className="absolute top-6 left-6 bg-black/60 backdrop-blur-md rounded-full px-4 py-2 border border-white/20">
                <div className="flex items-center space-x-3">
                  ...
                  <div className="text-white">
                    ...
                    <p className="text-xs text-white/70">Health Consultation</p>
                  </div>
                </div>
              </div>
      ...
      {localSessionId && (
        <div className="absolute bottom-32 right-6 w-48 h-36 sm:w-56 sm:h-40 lg:w-64 lg:h-44 z-30">
          <div className="relative w-full h-full rounded-2xl overflow-hidden shadow-2xl border-2 border-white/30 backdrop-blur-sm bg-black/30">
            <video
              ref={(videoElement) => {
                if (videoElement && daily && localSessionId) {
                  const track = daily.participants()[localSessionId]?.tracks?.video;
                  if (track?.persistentTrack) {
                    videoElement.srcObject = new MediaStream([track.persistentTrack]);
                  }
                }
        ...
        <div className="bg-gradient-to-t from-black/80 via-black/60 to-transparent p-8">
          <div className="flex items-center justify-center space-x-6">
            ...
            <div className="relative group">
              <button
                onClick={toggleAudio}
                className={`relative p-4 rounded-full transition-all duration-200 transform hover:scale-110 ${
                  isMicEnabled 
